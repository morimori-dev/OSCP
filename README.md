...
> ⚠️ **本リポジトリは正規の許可された環境（自身の所有環境・有償ラボ・CTF・ペンテスト案件）での利用を想定。無許可のシステムへの利用は禁止。**
...
| [`scripts/`](./scripts/) | 自前ヘルパー（ポートスキャン雛形、レポート生成等） |
...
```
1. Recon          → rustscan + nmap
2. サービス列挙    → §3 ツール群（プロトコル単位）
3. 攻撃面選定      → §4 Web / §3.x サービス固有
4. Foothold       → §5 リバースシェル
5. ローカル列挙    → §6 (Linux) / §7 (Windows)
6. PrivEsc        → SUID / sudo / Potato / カーネル
7. AD 横展開      → §8（BloodHound 経由でパス選定）
8. DA / ルート    → §8.4 DCSync で最終フック
```
